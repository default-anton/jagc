...
export class InMemoryRunStore implements RunStore {
  ...
  private readonly idempotencyToRun = new Map<string, string>();
  private readonly threadSessions = new Map<string, ThreadSessionRecord>();

  async init(): Promise<void> {}

  async createRun(message: MessageIngest): Promise<CreateRunResult> {
    const dedupeKey = message.idempotencyKey ? `${message.source}:${message.idempotencyKey}` : null;

    if (dedupeKey) {
      const existingRunId = this.idempotencyToRun.get(dedupeKey);
      if (existingRunId) {
        const run = this.runs.get(existingRunId);
        if (!run) {
          throw new Error(`missing in-memory run for ${existingRunId}`);
        }
    ...
    if (dedupeKey) {
      this.idempotencyToRun.set(dedupeKey, runId);
    }
  ...
  async markSucceeded(runId: string, output: RunOutput): Promise<void> {
    ...
    if (!run) {
      throw new Error(`cannot mark run ${runId} as succeeded: run not found`);
    }
  ...
  async markFailed(runId: string, errorMessage: string): Promise<void> {
    const run = this.runs.get(runId);
    if (!run) {
      throw new Error(`cannot mark run ${runId} as failed: run not found`);
    }
  ...
  async getThreadSession(threadKey: string): Promise<ThreadSessionRecord | null> {
    return this.threadSessions.get(threadKey) ?? null;
  }
  ...
  async upsertThreadSession(threadKey: string, sessionId: string, sessionFile: string): Promise<ThreadSessionRecord> {
    ...
    this.threadSessions.set(threadKey, record);
    return record;
  }
}
