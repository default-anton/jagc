  ...
  pi.on('before_agent_start', async (event, ctx) => {
    ...
    if (!event.systemPrompt.includes(runtimeContextMarker)) {
      const runtimeContextLines = [
        ...
        '- Scheduled tasks are one-off or recurring work that jagc performs by running stored instructions at scheduled times.',
        '- Use `jagc task` via the shell (`bash` tool) to create, inspect, and control scheduled tasks.',
        '- Start with `jagc task --help`; run `jagc task <subcommand> --help`.',
        '- Primary subcommands: `create`, `list`, `get`, `update`, `delete`, `run --now`, `enable`, `disable`.',
        '- Prefer `--json` output whenever available.',
        '- Execution model: task creation stores title/instructions/schedule; the execution thread is created lazily on first due/run-now execution and then reused.',
        '- After task mutations (create/update/delete/run/enable/disable), verify state with `jagc task get <taskId> --json` or `jagc task list --json`.',
