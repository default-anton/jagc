  ...
  pi.on('before_agent_start', async (event, ctx) => {
    ...
    if (!event.systemPrompt.includes(runtimeContextMarker)) {
      const runtimeContextLines = [
        ...
        '- Run a memory checkpoint each substantive turn: proactively capture durable preferences, constraints, decisions, and commitments into memory; skip ephemeral details.',
        '- Prefer updating existing memory notes over creating new files; keep updates terse and refresh `updated_at`.',
        '- If uncertainty is high or details are sensitive, ask one brief confirmation question before persisting memory.',
