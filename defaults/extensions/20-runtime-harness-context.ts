  ...
  pi.on('before_agent_start', async (event, ctx) => {
    ...
    if (!event.systemPrompt.includes(runtimeContextMarker)) {
      const runtimeContextLines = [
        ...
        '- Manage scheduled tasks via `jagc task ... --json` through the shell (`bash` tool). Do not modify jagc SQLite tables directly.',
        '- Scheduled-task command patterns: `jagc task create ... --json`, `jagc task list --json`, `jagc task get <taskId> --json`, `jagc task update <taskId> ... --json`, `jagc task delete <taskId> --json`, `jagc task run <taskId> --now --json`.',
        '- After each task mutation (create/update/delete/run-now/enable/disable), verify state with `jagc task get <taskId> --json` or `jagc task list --json`.',
        '- Task execution threads are created lazily on first due/run-now execution, not at task creation time.',
